...
int handleCONNECT(struct msgBuffer queryBuffer) {
    ...
    int client2 = queryBuffer.key;
    int errorCode1;
    int errorCode2;
    struct msgBuffer signalBuffer;
    signalBuffer.mtype = CONNECT;
    signalBuffer.id = clientQids[client2];
    strcpy(signalBuffer.text, "Server: You are now in chat!\n\0");
    errorCode1 = msgsnd(clientQids[client1], &signalBuffer, MSGBUF_RAW_SIZE, 0);
    if (errorCode1 == -1) {
        printf("%s\n", strerror(errno));
    }
    signalBuffer.id = clientQids[client1];
    errorCode2 = msgsnd(clientQids[client2], &signalBuffer, MSGBUF_RAW_SIZE, 0);
    if (errorCode2 == -1) {
        printf("%s\n", strerror(errno));
    }
    if (errorCode1 != -1 && errorCode2 != -1) {
        clientStatus[client1] = 0;
        clientStatus[client2] = 0;
        return 1;
    } else {
        return -1;
    }
