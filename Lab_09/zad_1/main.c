...
void *santa_thread_func(void *arg) {
    ...
    struct sembuf inc_elves_sem;
    inc_reindeer_sem.sem_num = 1;
    inc_reindeer_sem.sem_op = 3;
    for (int i = 0; i < N; i++) {
        pthread_mutex_lock(&santa_mutex);
        while (!deer_ready || !elves_waiting) {
        ...
        if (deer_ready) {
            ...
            santa_sleep = 0;
            for (int j = 0; j < 3; j++) {
                printf("Św. Mikołaj pomaga elfowi %d\n", elves[j]);
                sleep(1);
            }
            santa_sleep = 1;
...
void *reindeer_thread_func(void *arg) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        pthread_mutex_lock(&deer_mutex);
        while (santa_sleep) {
            pthread_cond_wait(&deer_cond, &deer_mutex);
        }
        pthread_mutex_unlock(&deer_mutex);
        pthread_mutex_lock(&deer_mutex);
        while (!santa_sleep) {
...
void *elves_thread_func(void *arg) {
    ...
    int sem_val;
    int sleep_time;
    struct sembuf dec_elves_sem;
    dec_elves_sem.sem_num = 1;
    dec_elves_sem.sem_op = -1;
    for (int i = 0; i < N; i++) {
        sleep_time = rand() % 3 + *my_id % 3;
        sleep(sleep_time);
        pthread_mutex_lock(&elves_mutex);
        semop(sem_id, &dec_elves_sem, 1);
        sem_val = semctl(sem_id, 0, GETVAL, NULL);
        elves[sem_val] = *my_id;
        pthread_mutex_unlock(&elves_mutex);
        printf("Elf %d: czeka %d elfów na Świętego Mikołaja\n", *my_id, 3 - sem_val);
        if (sem_val == 0) {
            pthread_mutex_lock(&elves_mutex);
            elves_waiting = 1;
            printf("Elf %d: wybudzam Św. Mikołaja\n", *my_id);
            pthread_cond_broadcast(&santa_cond);
            pthread_mutex_unlock(&elves_mutex);
        }
        pthread_mutex_lock(&elves_mutex);
...
int main() {
    ...
    sem_id = semget(ftok(getenv("HOME"), SEM_ID), 2, IPC_CREAT  | PERMISSIONS);
    if (sem_id == -1) {
        printf("%s\n", strerror(errno));
        raise(SIGINT);
    }
